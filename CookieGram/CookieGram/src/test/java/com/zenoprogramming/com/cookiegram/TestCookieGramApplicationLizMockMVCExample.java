...
public class TestCookieGramApplicationLizMockMVCExample
{
   ...
   {
      mockMvc.perform(MockMvcRequestBuilders.get("/ordercookie")).andExpect(status().isOk());
   }
   ...
   {
      mockMvc.perform(MockMvcRequestBuilders.get("/odercookie")).andExpect(status().isNotFound());
   }
